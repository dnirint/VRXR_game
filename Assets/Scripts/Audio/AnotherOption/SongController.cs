...
public class SongController : MonoBehaviour
{
    ...
    public UnityEvent OnProcessingEnded;
    public static SongController Instance { get; private set; } = null;
...
//			realTimePlotController = GameObject.Find ("RealtimePlot").GetComponent<PlotController> ();
    ...
    void Update()
    {
        ...
        if (realTimeSamples)
        {
            ...
            ShootToBeat(realTimeSpectralFluxAnalyzer.spectralFluxSamples);
//			realTimePlotController.updatePlot (realTimeSpectralFluxAnalyzer.spectralFluxSamples);
        }

        if (BackgroundProcessingDone)
        {
            OnProcessingEnded.Invoke();
            BackgroundProcessingDone = false;
        }
...
//            ShootToBeat(realTimeSpectralFluxAnalyzer.spectralFluxSamples, indexToPlot);
...
//        }
    ...
    private bool BackgroundProcessingDone = false;
    ...
    public void getFullSpectrumThreaded()
    {
        try
        {
            ...
            // Combine all samples into mono form
            ...
            for (int i = 0; i < iterations; i++)
            {
                ...
                preProcessedSpectralFluxAnalyzer.analyzeSpectrum(Array.ConvertAll(scaledFFTSpectrum, x => (float) x),
                    curSongTime);
            }

            BackgroundProcessingDone = true;
