...
public class SpectralFluxAnalyzer
{
	...
	float calculateRectifiedSpectralFlux() {
		...
		for (int i = 1; i < UpperHistogramBucket; i++) {
...
/*
 *  If you want to run the same algorithm over a subset of frequencies, just the sub-bass and bass range for example,
 *  it’s as simple as specifying which indexes of the 1024 length spectrum you want to process the rectified spectral flux for.
 *  Nothing else about the algorithm needs to change.
 */
